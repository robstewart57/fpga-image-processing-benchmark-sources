...
array<array<int, W>, H> blurFilter (array<array<int, W>, H> image) {
  for (int j=0;j<H;++j) {
    for (int i=0;i<W;++i) {
      int left  = i == 0 ? 0 : i - 1;
...
int main( int argc, char** argv )
{
  ...
  std::array<std::array<int, width>, height> img;
  // for (int i=0; i<height; ++i)
  //   img[i] = new int[width];
  ...
  std::array<std::array<int, width>, height> result = blurFilter<width,height>(img);
